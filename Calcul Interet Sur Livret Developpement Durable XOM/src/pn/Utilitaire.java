...
public class Utilitaire {
	...
	public static Date retrournerLaDateDeValeurDeLaDernière(Transaction tr) throws ParseException {
		Date date = convertiEnDate(tr.getDateDeLaTransactionPrecedente());
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);

		if (tr.getMontantPrecedent() > 0) { // La transaction est un débit
			if ((cal.get(Calendar.DAY_OF_MONTH) < 16)) {
				cal.set(Calendar.DAY_OF_MONTH, 16);
		...
		} else if (tr.getMontantPrecedent() < 0) {
			if ((cal.get(Calendar.DAY_OF_MONTH) < 16)) {
				cal.set(Calendar.DAY_OF_MONTH, 1);
